...
async function checkAuth() {
    ...
    // PRIORITY 2: Check Supabase session (if localStorage check failed)
    try {
        ...
        try {
            ...
            if (!userEmail && supabaseClient) {
                try {
                    ...
                    console.log('Supabase session not available:', err);
                }
            }
            ...
            localStorage.setItem('influora_user', JSON.stringify(updatedUser));
            ...
            // Try to save to Supabase if available (optional)
            if (supabaseClient) {
                try {
                    await supabaseClient.auth.updateUser({
                    ...
                    console.log('Supabase update skipped:', err);
                }
            }
